...
const SUPER_KEY_BASE = process.env.SUPER_KEY_BASE;
export default class Index extends React.Component {
  ...
  componentDidMount() {
    ...
    this.addChannel("realtime:public:associations");
    this.addChannel("realtime:public:utilisateurs");
    this.addChannel("realtime:public:utilisateurs:id=eq.2");
    this.fetchData();
  }
  addChannel(topic) {
  ...
  messageReceived(channel, msg) {
    ...
    if (channel === "realtime:public:utilisateurs") {
      this.setState({ utilisateurs: [...this.state.utilisateurs, msg.record] });
    }
    if (channel === "realtime:public:associations") {
    ...
    return (
      <div style={styles.main}>
        <div style={styles.row}>
          ...
          <div style={styles.col}>
            <h3>Utilisateurs</h3>
            <button onClick={() => this.insertUser()}>
              Ajouter un utilisateur au hasard
            </button>
          </div>
          <div style={styles.col}>
            <h3>Associations</h3>
            <button onClick={() => this.insertAsso()}>
              Ajouter une association au hasard
            </button>
          </div>
        </div>
        <div style={styles.row}>
